            ...
            try {
                bb.jquery(droppable).sortable({
                    ...
                    over: function(event, ui) {
                        ...
                        if (!canAccept)
                            bb.jquery(ui.placeholder).addClass(_settings.invalidPlaceHolderCls);
                        if (canAccept)
                            bb.jquery(ui.placeholder).removeClass(_settings.invalidPlaceHolderCls);
                        bb.jquery(document).trigger("content:overItem");
                        if (false == _paddingTopSaved)
                            _paddingTopSaved = bb.jquery(this).css('padding-top');
                        if (false == _paddingBottomSaved)
                            _paddingBottomSaved = bb.jquery(this).css('padding-bottom');
                            ...
                            if (draggedItem.parent()[0] == bb.jquery(this)[0] || fixContent) {
                                ...
                                if (dropType == "newContent") {
                                    ...
                                    var afterAppend = function() {
                                        if (bb.jquery(draggedItem).length)
                                            bb.jquery(draggedItem).show();
                                    }
...
(function($) {
    ...
    var ResizableItem = function(userSettings) {
        ...
        ResizableItem.prototype._onResizeStop = function(event, ui) {
            ...
            if (this._bbContent.parentNode) {
                parentparams = this._bbContent.parentNode.getContentParams();
                if (parentparams.contentwidth) {
                    if (!parentparams.contentwidth.array || !bb.jquery.isPlainObject(parentparams.contentwidth.array)) {
                        parentparams.contentwidth['array'] = {};
                    }

                    if (!parentparams.contentwidth.array[this._bbContent.getUid()]) {
                        parentparams.contentwidth.array[this._bbContent.getUid()] = '';
                    }
                    
                    parentparams.contentwidth.array[this._bbContent.getUid()] = this._settings.sizePrefix + this._bbContent.getSize();
                    this._bbContent.parentNode.set('param', parentparams, true);
                }
            }                   
        ...
        ResizableItem.prototype.setItemSize = function(itemSize) {
            if (itemSize) {
                ...
                if (bb.jquery(this._element).parentNode) {
                    console.log(bb.jquery(this._element).parentNode);
                }
            }
        }
