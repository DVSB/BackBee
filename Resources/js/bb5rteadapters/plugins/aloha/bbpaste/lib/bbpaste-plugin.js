...
define([
	 ...
	 **/
        ...
        var oldInsertHtml = alohaCore.commands.inserthtml;
        /**
         * We keep a reference of the fixHtmlCommand
         **/
        var oldInsertHtmlCommand = (function(){
            return function(value,range){
                oldInsertHtml.action(value,range);
            }
           
        })();
        
        /**
         * from Aloha-full.fs [29663]
         **/
        var insertHtmlCommand = {
            action: function (value, range) {
                oldInsertHtmlCommand(value,range);		
            }
        } 
	 ...
	 */
         ...
         **/
        function showConfirmDialog(content,$clipBoard,range){
            var popupMng = bb.PopupManager.init({});
            /* create dialog up */
            var dialogContent = $("<div><strong>Preserve the content's style?</strong></div>");
            if(!confirmDialog){
			
                popupMng.registerDialogType("bbpaste-plugin","<div class='bb5-ui bb5-dialog-bbpaste-plugin'></div>");
                confirmDialog = popupMng.create("bbpaste-plugin",{
                    title: "Format Dialog",
                    modal: false,
                    height: 150,
                ...
                confirmDialog.addButton({
                    text:"Yes", 
                    click : (function(formatConf){
                        return function(){
                            Aloha.settings.contentHandler.insertHtml = formatConf;
                            confirmDialog.close(); 
                        }
                       
                    })(pluginConfig.formatFilters)
                });
                confirmDialog.addButton({
                    text:"No", 
                    click : (function(formatConf){
                    ...
                    })(pluginConfig.formatlessFilters)
                });
                confirmDialog.setContent(dialogContent);
                confirmDialog.on("close", function(e){
                    var $clipboard = confirmDialog.getExtra("clipboard");
                    if (Aloha.queryCommandSupported('insertHTML')) {
                        Aloha.activeEditable = confirmDialog.getExtra("currentEditable"); 
                        var range = confirmDialog.getExtra("range"); 
                        restoreSelection(range);
                        prepRangeForPaste(range); 
                        setTimeout(function(){
                            Aloha.execCommand('insertHTML', false, confirmDialog.getExtra("content"))
                        },0);
                    } else {
                        Console.error(
                            'Common.Paste',
            ...
            confirmDialog.setExtra("range",range);
            confirmDialog.setExtra("currentEditable",Aloha.getActiveEditable());
            confirmDialog.setExtra("content",content);
            confirmDialog.setExtra("clipboard",$clipBoard);
            confirmDialog.show();
	 ...
	 */
            ...
            return Plugin.create('bbpaste', {
                ...
                init: function () {
                    ...
                    pluginConfig  =  $.isPlainObject(this.settings.config) ? this.settings.config : {};
                    ...
                    if (IS_IE) {
                        $CLIPBOARD.bind('paste', function ($event) {
                            ...
                            onPaste($event, ieRangeBeforePaste,pluginConfig,function () {
