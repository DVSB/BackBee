    ...
    protected function _extractDatas($datas)
    {
        ...
        foreach($datas as $key => $value) {
            ...
            if (is_array($value)) {
                if (isset($value['type'])) {
                    $type = $value['type'];
                    if (isset($value['default'])) $options['default'] = $value['default'];
                    if (isset($value['label'])) $options['label'] = $value['label'];
                    if (isset($value['maxentry'])) $options['maxentry'] = $value['maxentry'];
                    if (isset($value['parameters'])) $options['parameters'] = $this->_extractDatas($value['parameters']);
                } else {
                    $type = 'array';
                ...
                if (strpos($value, '!!') === 0) {
                    ...
                    if (isset($typedValue[1])) $options['default'] = $typedValue[1];
                }
            }
