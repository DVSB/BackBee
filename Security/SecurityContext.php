    ...
    public function createFirewall($name, $config) {
        ...
        if (array_key_exists('requirements', $config)) {
            foreach($config['requirements'] as $key => $value) {
                if (0 === strpos($key, 'HTTP-')) $requestMatcher->matchHeader(substr($key, 5), $value);
            }
        }
        ...
        if (array_key_exists('logout', $config)) {
            $listener = new LogoutListener($this, $httpUtils = new HttpUtils(), new LogoutSuccessHandler($httpUtils));
            if (array_key_exists('handlers', $config['logout']) && is_array($handlers = $config['logout']['handlers']))
            {
                foreach ($handlers as $handler)
                    $listener->addHandler(new $handler());
            }
            $this->_dispatcher->addListener('frontcontroller.request.logout', array($listener, 'handle'));
        }
