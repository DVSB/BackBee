...
class FrontController implements HttpKernelInterface
{
    ...
    private function _flushfile($filename)
    {
        if (FALSE === file_exists($filename) || FALSE === is_readable($filename))
            throw new FrontControllerException(sprintf('The file `%s` can not be found.', $this->_request->getPathInfo()), FrontControllerException::NOT_FOUND);
    ...
    public function defaultAction($uri = NULL, $sendResponse = true)
    {
        ...
        if (NULL === $page)
            throw new FrontControllerException(sprintf('The URL `%s` can not be found.', $uri), FrontControllerException::NOT_FOUND);
        ...
        try {
            ...
            throw new FrontControllerException(sprintf('An error occured while rendering URL `%s`.', $uri), FrontControllerException::INTERNAL_ERROR, $e);
        }
    }
